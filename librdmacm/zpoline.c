...
/*
 * zpoline related code for rsocket
 * Copyright (c) 2024 Hajime Tazaki
 *
...
static int transpose_socket(int socket, enum fd_type new_type)
{
	...
	if (new_type == fd_rsocket) {
		...
		dapi = &real;
		sapi = &rs;
		dprintf("fall back to real socket (%s)\n", __func__);
	}
...
int hijack_socket(int domain, int type, int protocol)
{
...
int hijack_bind(int socket, const struct sockaddr *addr, socklen_t addrlen)
{
	int fd;
...
int hijack_listen(int socket, int backlog)
{
	int fd, ret;
...
int hijack_accept(int socket, struct sockaddr *addr, socklen_t *addrlen)
{
	int fd, index, ret;
...
int hijack_connect(int socket, const struct sockaddr *addr, socklen_t addrlen)
{
	int fd, ret;
...
ssize_t hijack_recv(int socket, void *buf, size_t len, int flags)
{
	int fd;
...
ssize_t hijack_recvfrom(int socket, void *buf, size_t len, int flags,
		 struct sockaddr *src_addr, socklen_t *addrlen)
{
	int fd;
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_recvmsg(int socket, struct msghdr *msg, int flags)
{
	int fd;
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_read(int socket, void *buf, size_t count)
{
	int fd;
	init_preload();
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_readv(int socket, const struct iovec *iov, int iovcnt)
{
	int fd;
	init_preload();
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_send(int socket, const void *buf, size_t len, int flags)
{
	int fd;
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_sendto(int socket, const void *buf, size_t len, int flags,
		const struct sockaddr *dest_addr, socklen_t addrlen)
{
	int fd;
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_sendmsg(int socket, const struct msghdr *msg, int flags)
{
	int fd;
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_write(int socket, const void *buf, size_t count)
{
	int fd;
	init_preload();
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
ssize_t hijack_writev(int socket, const struct iovec *iov, int iovcnt)
{
	int fd;
	init_preload();
	return (fd_fork_get(socket, &fd) == fd_rsocket) ?
...
int hijack_shutdown(int socket, int how)
{
	int fd;
	return (fd_get(socket, &fd) == fd_rsocket) ?
...
int hijack_getpeername(int socket, struct sockaddr *addr, socklen_t *addrlen)
{
	int fd;
	return (fd_get(socket, &fd) == fd_rsocket) ?
...
int hijack_getsockname(int socket, struct sockaddr *addr, socklen_t *addrlen)
{
	int fd;
	init_preload();
	return (fd_get(socket, &fd) == fd_rsocket) ?
...
int hijack_setsockopt(int socket, int level, int optname,
		const void *optval, socklen_t optlen)
{
	int fd;
	return (fd_get(socket, &fd) == fd_rsocket) ?
...
int hijack_getsockopt(int socket, int level, int optname,
		void *optval, socklen_t *optlen)
{
	int fd;
	return (fd_get(socket, &fd) == fd_rsocket) ?
...
int hijack_fcntl(int socket, int cmd, ... /* arg */)
{
	va_list args;
	long lparam;
	void *pparam;
 ...
 */
int hijack_dup2(int oldfd, int newfd)
{
...
ssize_t hijack_sendfile(int out_fd, int in_fd, off_t *offset, size_t count)
{
	void *file_addr;
	int fd;
	size_t ret;
...
int hijack___fxstat(int ver, int socket, struct stat *buf)
{
	int fd, ret;
...
#define CALL_RSOCKET_FD_SYSCALL1(x)				\
{								\
	case __NR_##x:						\
		ret = hijack_##x(a2);				\
		break;						\
...
#define CALL_RSOCKET_FD_SYSCALL2(x)				\
{								\
	case __NR_##x:						\
		ret = hijack_##x(a2, a3);			\
		break;						\
...
#define CALL_RSOCKET_FD_SYSCALL3(x)				\
{								\
	case __NR_##x:						\
		ret = hijack_##x(a2, a3, a4);			\
		break;						\
}

#define CALL_RSOCKET_FD_SYSCALL4(x)				\
{								\
	case __NR_##x:						\
		ret = hijack_##x(a2, a3, a4, a5);		\
		break;						\
}

#define CALL_RSOCKET_FD_SYSCALL5(x)				\
{								\
	case __NR_##x:						\
		ret = hijack_##x(a2, a3, a4, a5, a6);		\
		break;						\
}

#define CALL_RSOCKET_FD_SYSCALL6(x)				\
{								\
	case __NR_##x:						\
		ret = hijack_##x(a2, a3, a4, a5, a6, a7);	\
		break;						\
}


long zpoline_rsocket_hook(int64_t a1, int64_t a2, int64_t a3,
...
//	dprintf("syscall %ld: tid: %d\n", a1, gettid());
	...
	switch (a1) {
		CALL_RSOCKET_FD_SYSCALL3(sendmsg);
		CALL_RSOCKET_FD_SYSCALL3(recvmsg);
#if 0
		CALL_RSOCKET_FD_SYSCALL4(sendmmsg);
		CALL_RSOCKET_FD_SYSCALL4(recvmmsg);
#endif
		CALL_RSOCKET_FD_SYSCALL3(bind);
		CALL_RSOCKET_FD_SYSCALL3(connect);
		CALL_RSOCKET_FD_SYSCALL5(getsockopt);
		CALL_RSOCKET_FD_SYSCALL5(setsockopt);
		CALL_RSOCKET_FD_SYSCALL3(getsockname);
		CALL_RSOCKET_FD_SYSCALL6(sendto);
		CALL_RSOCKET_FD_SYSCALL6(recvfrom);
		CALL_RSOCKET_FD_SYSCALL2(listen);
		CALL_RSOCKET_FD_SYSCALL3(accept);
		CALL_RSOCKET_FD_SYSCALL1(close);
#if 0
		CALL_RSOCKET_FD_SYSCALL3(ioctl);
#endif
		CALL_RSOCKET_FD_SYSCALL3(fcntl);
		CALL_RSOCKET_FD_SYSCALL3(read);
		CALL_RSOCKET_FD_SYSCALL3(write);
#if 0
		CALL_RSOCKET_FD_SYSCALL4(pread64);
#endif
		CALL_RSOCKET_FD_SYSCALL3(socket);
#if 0
	case __NR_openat:
		if (!lkl_running)
...
int __hook_init(long placeholder __attribute__ ((__unused__)),
		void *default_hook,
		long memcheck __attribute__ ((__unused__)))
{
	*((syscall_fn_t *) default_hook) = zpoline_rsocket_hook;
	return 0;
}
